#!/bin/bash
#
# Wrapper for tilt that uses the bazel kubectl implementation.
#
set -euo pipefail
...
export PATH="$(dirname "$(rlocation com_github_kubernetes-sigs_kustomize/kustomize)"):${PATH}"

# Switch to original working directory and execute tilt locally.
pushd "$BUILD_WORKING_DIRECTORY"
exec $(rlocation com_github_tilt-dev_tilt/tilt) $@
